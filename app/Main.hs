...
main = do
  ...
  case names of
    [] -> printUsage progName
    [name] -> do
      createAWindow "FRAG" name
      mainLoop
    _ -> printUsage progName

printUsage :: String -> IO ()
printUsage n = do
  putStrLn $ "Usage: " ++ n ++ " <level>"
...
createAWindow windowName level = do
  ...
  (_, _) <-
    getWinInput
      (lasttime, (newInput, newMouseInput))
      inpState
      True
      tme
...
getWinInput ::
  ...
  Bool ->
  Int ->
  IO (DTime, Maybe (WinInput, WinInput))
