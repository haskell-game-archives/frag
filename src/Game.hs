...
clip mp gi cam@OOSCamera {newCam = cam1, oldCam = cam2} ooses k msgs =
  let (camera, grounded) = clipCamera mp cam1 cam2
   in ObjInput
        { oiHit = listToEvent $ findCollidingObjects ooses (k, cam),
...
clip
  ...
  k
  msgs =
    let (clippedPos, grounded) = clipObject mp pos1 pos2 sz
     in ObjInput
          { oiHit =
...
clip
  ...
  msgs
    | not ff =
      let (_, hasCol) = clipRay mp pos3 pos1 (0, 0, 0)
       in ObjInput
            { oiHit = noEvent,
              oiMessage = findMessages k msgs,
              oiCollision = dummy,
...
clip
  mp
  gi
  projectile@OOSProjectile
    { projectileNewPos = pos3,
...
tryAccurate mp n dv1 dv2 vec
  ...
  where
    middle v1 v2 _ = vectorAdd v1 (vectorMult (vectorSub v2 v1) 0.5)
...
checkCollision obj1 obj2
  | isCamera obj2 && isProjectile obj1 =
    pointBox
      (cpos (oldCam obj2))
      (projectileOldPos obj1)
      (20, 50, 20)
  | isCamera obj1 && isProjectile obj2 =
    pointBox
      (cpos (oldCam obj1))
      (projectileOldPos obj2)
      (20, 50, 20)
...
findVisibleTargets bsp ooses (k, obj) range
  | isAICube obj =
    [ (k', oos') | (k', oos') <- ooses, k /= k', isCamera oos', checkVisible bsp obj oos' range
    ]
  | otherwise = []
