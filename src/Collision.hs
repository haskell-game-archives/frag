...
clipCamera mp newcam oldcam =
  let (colPos, _, ground, nsteps) =
        traceStep
          (createBox (-20, -50, -20) (20, 50, 20))
          mp
          (cpos oldcam)
          (cpos newcam)
...
traceStep cType mp start end
  ...
  where
    (newPos, col, step, ground, _) = trace cType mp start end
...
traceo cType mp start end
  | newRatio /= 1.0 =
    ( vectorAdd
        start
        (vectorMult (vectorSub end start) newRatio),
      hasCol,
      step,
      grounded
    )
