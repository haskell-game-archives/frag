...
renderBSP' leaf mp = do
  ...
  renderBSPCleanUp
  where
    visFunc = isClusterVisible (visData mp) (cluster leaf)
...
renderMeshFace face (vertexPtr, texturePtr, c, _, _) vIndex = do
  ...
  drawRangeElements
    Triangles
    (0, fromIntegral (numOfVerts face))
    (numOfIndices face)
    UnsignedInt
    (plusPtr vIndex (4 * startIndex face))
...
renderPatch face bsppatch = do
  ...
  multiDrawElements
    TriangleStrip
    (numIndexPtr bsppatch)
    UnsignedInt
    (indexPtrPtr bsppatch)
...
readFaces handle lumps vertArrays indcs = do
  ...
  mapM
    (readFace handle offst lightMapArray texObjArray vertArrays indcs)
    offs

readFace ::
  Handle ->
  Int ->
  Array Int TextureObject ->
  Array Int (Maybe TextureObject) ->
  VertexArrays ->
  Ptr GLint ->
  Int ->
  IO BSPFace
...
seperateArrays ::
  [([CFloat], [CFloat], [CFloat], [CFloat], [Word8])] ->
